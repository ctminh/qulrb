...
if __name__ == "__main__":
    ...
    data_balanced_imbs = {'num_procs': num_procs}
    data_balanced_speedups = {'num_procs': num_procs}
    data_total_num_mig_tasks = {'num_procs': num_procs}
    data_avg_num_mig_tasks_per_proc = {'num_procs': num_procs}
    ...
    for algorithm in list_algorithms:
        ...
        for npro in num_procs:
            # identify the input and output files
            input = input_folder + 'samoa_osc_case_' + str(npro) + 'procs.csv'
            output = output_folder + 'output_' + algorithm + '_lrp_' + str(npro) + 'nodes.csv'
            ...
            if os.path.isfile(input) and os.path.isfile(output):
                ...
                print("-------------------------------------------")
                print(" + Rimb before rebalancing: ", Rimb_input)
                print(" + Rimb after  rebalancing: ", Rimb_output)
                print("-------------------------------------------\n")
            else:
    ...
    print("-------------------------------------------")
    print("Write the table of imb ratios after rebalancing to csv: ./data_samoa_osc_imbs.csv")
    df_imb_ratios.to_csv('./data_samoa_osc_imbs.csv', index=False)
    print("-------------------------------------------\n")

    print("Write the table of speedup after rebalancing to csv: ./data_samoa_osc_speedups.csv")
    df_speedups.to_csv('./data_samoa_osc_speedups.csv', index=False)
    print("-------------------------------------------\n")
