...
def generate_df_by_iter(df_header, dat_src, dat_idx, num_iters):
    ...
    for i in range(num_iters):
        ...
        for r in range(num_ranks):
            ptr_to_data_arr = dat_src[r][dat_idx]
            datarow.append(ptr_to_data_arr[i])
...
if __name__ == "__main__":
    ...
    print("---------------------------")
    # print(" + Write dataframe to csv: ./dataframe/df_runtime.csv")
    # df_runtime.to_csv('./dataframe/df_runtime.csv', index=False)
    # print("---------------------------")


    # print("\n-----------------------------------------------------")
    # print("Dataframe for balancing ops overhead")
    # print("-----------------------------------------------------")
    # global_avg_num_calls = 0.0
    # global_avg_bal_cost  = 0.0

    # print("---------------------------")
    # print(" + Num. balancing ops cals:")
    # print("---------------------------")
    # df_num_balancing_cals = generate_df_by_iter(df_header, dataset, 14, num_iters)
    # print(df_num_balancing_cals)

    # print("---------------------------")
    # print(" + Cost. balancing ops:")
    # print("---------------------------")
    # df_num_balancing_cost = generate_df_by_iter(df_header, dataset, 15, num_iters)
    # print(df_num_balancing_cost)

    # df_avg_num_balancing_cals = df_num_balancing_cals[df_header[1:]].mean()
    # df_avg_num_balancing_cost = df_num_balancing_cost[df_header[1:]].mean()
    # print("---------------------------")
    # print(" + Averaging balancing overhead:")
    # print("---------------------------")
    # print("   * avg num-balancing cals:")
    # print(df_avg_num_balancing_cals)
    # print("   * avg num-balancing cost:")
    # print(df_avg_num_balancing_cost)
    # print("---------------------------")
    ...
    print("\n--------------------------------------------------------")
    print("Dataframe/Table for LRP input")
    print("--------------------------------------------------------")
    table_header = ['Process']
    table_data   = []
    num_local_tasks = df_num_local_tasks['R0'][0]
    for r in range(num_ranks):
        table_header.append('P' + str(r))
        datarow = ['P' + str(r)]
        for i in range(num_ranks):
            if i == r:
                datarow.append(num_local_tasks)
            else:
                datarow.append(0)
        avg_total_load_per_rank = 1000 * np.average(df_num_local_load['R'+str(r)]) # in ms
        avg_load_per_task = avg_total_load_per_rank / num_local_tasks # in ms
        datarow.append(avg_load_per_task)
        datarow.append(avg_total_load_per_rank)
        # add an entire row of the dataframe
        table_data.append(datarow)

    # add the header for load/task and total_load/process
    table_header.append('w')
    table_header.append('L')
    # generate dataframe
    df_input_lrp = pd.DataFrame(table_data, columns=table_header)
    print(df_input_lrp)
    # write to csv input file
    print(" + Write table for LRP input to csv: ./input_lrp/input_table_" + str(num_procs) + "nodes" + ".csv")
    df_input_lrp.to_csv('./input_lrp/input_table_' + str(num_procs) + 'nodes' + '.csv', index=False)
    print("---------------------------\n")
